...
from .database import init_db, get_db, SessionLocal, ChatSession, Message
...
async def chat_endpoint(
    ...
    image_files: List[UploadFile] = File(None), # ★ List[UploadFile] に変更
...
# ★ 画像の複数保存処理
        ...
        # 3. ルーターAI判定
        router_result = check_domain_and_categorize(user_text, image_bytes_list)
        if not router_result.get("is_subject_related"):
            return JSONResponse(
                ...
                content={
                    ...
                    "message": "私は中学受験の家庭教師です。専門外の質問にはお答えできません。"
                }
            )
        ...
        async def event_generator():
            ...
            for chunk in generate_answer_stream(user_text, image_bytes_list, chat_history):
                full_text += chunk
...
def get_all_sessions(db: Session = Depends(get_db)):
    ...
    # ★ 修正: SQLAlchemyのオブジェクトをそのまま返さず、確実に辞書のリストに変換する
    return [
        {
            "id": s.id,
            "category": s.category,
